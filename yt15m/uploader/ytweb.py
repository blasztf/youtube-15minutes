...
class YoutubeWebUploader(Uploader):
    ...
    def upload_video(self, video_model: VideoModel):
        ...
        try:
            ...
            self.youtube.get("https://studio.youtube.com")
            self.youtube.switch_to.alert.accept()
        except:
            pass
        ...
        try:
            ...
            self.youtube.find_elements(By.XPATH, "//ytcp-button[@id='create-icon']")[0].click_pro()
            helper.log(debug_text, True)

            debug_text = 'Click "Upload video" dropdown item.'
            self.youtube.find_elements(By.XPATH, "//tp-yt-paper-item[@id='text-item-0']")[0].click_pro()
            helper.log(debug_text, True)

            debug_text = 'Input file path to input file field (on changed, this will trigger submit file automatically).'
            self.youtube.find_elements(By.XPATH, "//*[@id='content']/input[@name='Filedata']")[0].send_keys(video_model.file)
            helper.log(debug_text, True)
            ...
            processing_badge_el = self.youtube.find_elements(By.XPATH, xpath)[0]
            xpath = "/html/body/ytcp-uploads-dialog"
            uploads_dialog_el = self.youtube.find_elements(By.XPATH, xpath)[0]
            ...
            while True:
                ...
                if has_attribute(uploads_dialog_el.get_attribute('has-error')):
                    ...
                    err_short_msg = self.youtube.find_elements(By.XPATH, xpath)[0].text
                ...
                if processing_badge_el.value_of_css_property('fill') == "rgb(62, 166, 255)":
                    ...
                    helper.log("Uploaded. Getting self.youtube.video id.", True)
                    video_id = self.youtube.find_elements(By.XPATH, "//*[@id='details']/ytcp-video-metadata-editor-sidepanel/ytcp-video-info/div/div[2]/div[1]/div[2]/span/a")[0].get_attribute('href')
                    break
            ...
            self.youtube.find_elements(By.XPATH, xpath)[0].click_pro()
            helper.log(debug_text, True)

            # Type video title.
            debug_text = "Input video title."
            self.youtube.switch_to.active_element.send_keys(Keys.CONTROL, 'a')
            self.youtube.switch_to.active_element.send_keys(video_model.title)
            helper.log(debug_text, True)
            ...
            self.youtube.switch_to.active_element.send_keys(video_model.description)
            helper.log(debug_text, True)
            ...
            self.youtube.find_elements(By.XPATH, xpath)[0].send_keys(video_model.keywords)
            helper.log(debug_text, True)
    ...
    def rewrite_description(self, video_id: str, video_model: VideoModel):
        ...
        try:
            ...
            self.youtube.get(f"https://studio.youtube.com/video/{video_id}/edit")
            self.youtube.switch_to.alert.accept()
        except:
            pass
        ...
        try:
            ...
            el = self.youtube.find_elements(By.XPATH, xpath)[0]
            el.click_pro()
            el.send_keys(Keys.CONTROL, 'a')
            el.send_keys(video_model.description)
            helper.log(debug_text, True)
